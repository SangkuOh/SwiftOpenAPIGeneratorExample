...
- 네트워크는 생성된 `Client`·`APIProtocol`을 `APIEnvironment`와 엔드포인트 모듈로 감싸 전송 세부 사항을 UI/도메인이 알 필요 없게 했습니다.
- OpenAPI 스펙(`Networking/Contract/openapi.yaml`)과 생성 설정(`Networking/Contract/openapi-generator-config.yaml`)만 관리하고, 생성물은 빌드 시 플러그인이 준비하므로 커밋 대상이 아닙니다.
- `MockServerTransport`로 네트워크 없이도 동일 코드 경로를 실행하는 프리뷰/테스트 구성을 제공합니다.

## 요구 사항
- Xcode 16 이상(스위프트 테스팅, Swift 6 타겟)과 iOS 17 이상 시뮬레이터 또는 디바이스.
- 기본 스펙은 `http://localhost:8080/api`에 열린 `GET /greet`가 `{"message":"..."}` 형태로 응답한다고 가정합니다. 다른 주소라면 `Config/*.xcconfig`와 `Networking/Contract/openapi.yaml`의 `servers`를 함께 조정하세요.
...
3) **커스텀 스펙 적용**: `Networking/Contract/openapi.yaml`과 `Networking/Contract/openapi-generator-config.yaml`를 원하는 계약/옵션으로 수정하고 빌드하면 플러그인이 자동 재생성합니다. 생성물은 DerivedData 아래에서만 유지됩니다.
...
- `Data/DefaultGreetingRepository.swift`: 엔드포인트 모듈 호출 후 도메인 엔터티로 매핑.
- `Environment/`: `AppDependencies` 조립(라이브/목), `EnvironmentValues` 확장.
- `Networking/Configuration/APIConfiguration.swift`: Info.plist·OpenAPI 서버 기반 구성/검증.
- `Networking/Errors/RemoteAPIError.swift`: OpenAPI 응답·트랜스포트 에러 매핑.
- `Networking/Environment/APIEnvironment.swift`: 생성된 `Client` 조립과 엔드포인트 모듈 제공.
- `Networking/Endpoints/GreetingEndpoint.swift`: 생성된 `Client`의 Greeting 엔드포인트 모듈.
- `Networking/Transport/MockServerTransport.swift`: 요청 기록·매칭 기반 목 트랜스포트와 `greetingResponse` 기본 스텁.
- `Networking/Contract/openapi.yaml` / `openapi-generator-config.yaml`: OpenAPI 계약과 생성 설정의 단일 소스.
...
swift openapi generate \
  --config SwiftOpenAPIGeneratorExample/Networking/Contract/openapi-generator-config.yaml \
  --output .openapi-generator \
  SwiftOpenAPIGeneratorExample/Networking/Contract/openapi.yaml
```

- 새 엔드포인트를 쓰려면 스펙에 `operationId`를 추가 → `Networking/Endpoints`에 모듈 추가/확장 → `APIEnvironment` 프로퍼티로 노출 → 리포지터리/서비스/뷰에서 사용 순으로 확장합니다.
...
## 참고
- 더 자세한 흐름과 결정 포인트는 `Documentation.docc/*` 문서를, 네트워크 계약은 `Networking/Contract/openapi.yaml`을 확인하세요.
