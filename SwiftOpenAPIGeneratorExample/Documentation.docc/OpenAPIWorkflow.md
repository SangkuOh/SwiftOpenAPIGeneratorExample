...
## 스펙
- 계약은 `Networking/Contract/openapi.yaml`에 정의되어 있습니다. 이 예제는 `GET /greet`가 `{"message": "..."}`를 반환한다는 단순 계약을 사용합니다.
...
## 생성 설정
- `Networking/Contract/openapi-generator-config.yaml`는 `types`와 `client`를 생성하고, `internal` 접근 제어 및 `idiomatic` 이름 전략을 사용하도록 지정합니다.
...
3. 엔드포인트 모듈(`Networking/Endpoints/*`)을 추가하거나 확장하고, `APIEnvironment`에서 프로퍼티로 노출해 리포지터리에서 호출할 수 있게 합니다. 필요하면 도메인 모델도 추가합니다.
